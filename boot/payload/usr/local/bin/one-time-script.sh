#!/bin/bash
# Start script for unattended configuration of a DEEPi OS
# 
# Original from https://gitlab.com/JimDanner/pi-boot-script
#
# NOTE: deepi modifications tagged with a '# NOTE:' code tag
...
get_parameters() {
    ...
    new_timezone='Europe/London' # this is UTC
    new_hostname_tag=''
    new_ssh_setting=0
    new_wifi_country=US
    ...
    # ...then see if values can be read from a file then remove that
    # (may contain password) but save parameters for the next script
    # back to the file
...
os_config() {
    ...
    # NOTE: original hostname section
    ...
    # NOTE: modified hostname section for DEEPi
    modelnr=$(sed -E 's/Raspberry Pi ([^ ]+).*/\1/' /proc/device-tree/model);
    serial=$(grep ^Serial /proc/cpuinfo | sed -E 's/^.*: .{10}//');
